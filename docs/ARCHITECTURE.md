...
`lib/config.ex`, `lib/config/schema.ex`, `lib/config/bootstrap.ex`, `lib/config/migrator.ex`, `lib/config/profiles.ex`, and `lib/config/auth.ex` are the local state layer. `RR.Config` owns the effective home directory and a few compatibility helpers. `RR.Config.Schema` declares the current on-disk config version, `RR.Config.Migrator` and `RR.Config.Migrations.V0ToCurrent` translate legacy config layouts into that version, and `RR.Config.Bootstrap` runs that migration check at CLI startup for stateful commands. `RR.Config.Profiles` now assumes config is already on the current schema and only owns profile-scoped reads, writes, and alias storage. `RR.Config.Auth` builds the auth struct, validates tokens, and keeps a short-lived in-memory ETS cache named `:rr_auth_cache` so repeated validations do not keep hitting Rancher.

`lib/external/` is the main boundary layer. `External.Config` abstracts config file reads, error-aware reads for bootstrap, backup creation, and writes. `External.RancherHttpClient` abstracts Rancher API calls for cluster lookup, kubeconfig generation, and token inspection; cluster and kubeconfig calls now take an explicit `%RR.Config.Auth{}` so command modules can target one profile or fan out across many profiles. The `impl.ex` files provide the default runtime implementations. Tests replace these boundaries through `Application.get_env(:rr, :external_bound, ...)` and the Mox mocks declared in `test/test_helper.exs`.
...
`RR.Config.Bootstrap` is the only place that should reason about historical config versions or rewrite `config.json` during startup. `RR.Config.Profiles` should only handle the current schema shape and should limit its normalization to current-version conveniences such as defaulting missing alias maps. Command modules should work in terms of profile names, `%RR.Config.Auth{}` structs, and alias helper functions rather than reaching into raw config maps directly.
